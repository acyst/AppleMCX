/*
 * ib_read_lat.c — RDMA Read latency test (macOS) — perftest equivalent
 * Usage: ib_read_lat [server|client]
 */
...
int main(int argc, char **argv)
{
    ...
    if (pt_init(&ctx, argc, argv) != 0)
        return 1;
    printf("ib_read_lat (macOS) - RDMA Read latency test\n");
